...
const DetailGallery = () => {
  const filePaths = React.useMemo(
    () => ['/img/product/small/product-2.jpg', '/img/product/small/product-3.jpg', '/img/product/small/product-4.jpg', '/img/product/small/product-5.jpg'],
    []
  );
    ...
    return new Promise((resolve, reject) => {
      fetch(path).then((res) => {
        res.arrayBuffer().then((buf) => {
          return resolve(new File([buf], 'image_data_url.jpg', { type: 'image/jpeg' }));
        });
      });
    });
  };
